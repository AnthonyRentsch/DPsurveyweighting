...
pums_file_suffix = " _pums.csv";
pumsfile_path = "./data/state_pums/ ";
...
get_acs_data <- function(state){
  file_path <- paste0(pumsfile_path, state, pums_file_suffix);
  print(file_path)
  data <- read.csv(file_path);
  return(data)
}

test <- get_acs_data("ca")
