...
###
# our true weights
true.weighted.res.vs <- as.data.frame(svytable(~preference+education, design=cces16.des.ps)) %>% 
...
unweighted.res.vs <- as.data.frame(svytable(~preference+education, design=cces16.unweighted.des)) %>% 
  mutate(education = case_when(education == 1 ~ "No HS",
                               education == 2 ~ "HS graduate",
                               education == 3 ~ "Some college",
                               education == 4 ~ "2-year degree",
...
results.vs <- matrix(NA, nrow=length(epsilons)*num_iter*length(unique(cces16_slim$education)), ncol=6)
for (epsilon in epsilons) {
  ...
  for (iter in 1:num_iter) {
    ...
    noisy.weighted.res.vs <- as.data.frame(svytable(~preference+education, design=cces16.noisy.des.ps)) %>% 
      mutate(education = case_when(education == 1 ~ "No HS",
                                   education == 2 ~ "HS graduate",
                                   education == 3 ~ "Some college",
                                   education == 4 ~ "2-year degree",
    ...
    for (education in unique(noisy.weighted.res.vs$education)) {
      clinton.noisy.share <- noisy.weighted.res.vs$share[noisy.weighted.res.vs$education==education & noisy.weighted.res.vs$preference=="Clinton"]
      trump.noisy.share <- noisy.weighted.res.vs$share[noisy.weighted.res.vs$education==education & noisy.weighted.res.vs$preference=="Trump"]
      noisy.dif <- clinton.noisy.share - trump.noisy.share
        
      clinton.true.share <- true.weighted.res.vs$share[true.weighted.res.vs$education==education & true.weighted.res.vs$preference=="Clinton"]
      trump.true.share <- true.weighted.res.vs$share[true.weighted.res.vs$education==education & true.weighted.res.vs$preference=="Trump"]
      true.dif <- clinton.true.share - trump.true.share
      
      clinton.unweighted.share <- unweighted.res.vs$share[unweighted.res.vs$education==education & unweighted.res.vs$preference=="Clinton"]
      trump.unweighted.share <- unweighted.res.vs$share[unweighted.res.vs$education==education & unweighted.res.vs$preference=="Trump"]
      unweighted.dif <- clinton.unweighted.share - trump.unweighted.share
      
      results.vs[i,] <- c(education, epsilon, iter, noisy.dif, true.dif, unweighted.dif)
      i <- i + 1
    }
  }
}
...
names(results.vs.df) <- c("education", "epsilon", "iteration", 
...
avg.results.vs.df <- results.vs.df %>% group_by(epsilon, education) %>% 
...
vote_share_difs_education <- ggplot(data=avg.results.vs.df, aes(x=epsilon)) + 
...
###
# our true weights
true.weighted.res.arb <- as.data.frame(svytable(~assault_rifle_ban+race, design=cces16.des.ps)) %>% 
...
# unweighted
unweighted.res.arb <- as.data.frame(svytable(~assault_rifle_ban+race, design=cces16.unweighted.des)) %>% 
  mutate(race = case_when(race == 1 ~ "White",
                          race == 2 ~ "Black",
                          race == 3 ~ "Hispanic",
                          race == 4 ~ "Asian",
...
results.arb <- matrix(NA, nrow=length(epsilons)*num_iter*length(unique(cces16_slim$race)), ncol=6)
for (epsilon in epsilons) {
  cat("\nEpsilon:", epsilon)
  for (iter in 1:num_iter) {
    ...
    # compute cross-tab for support of rifle ban based on race 
    noisy.weighted.res.arb <- as.data.frame(svytable(~assault_rifle_ban+race, design=cces16.noisy.des.ps)) %>% 
      mutate(race = case_when(race == 1 ~ "White",
                              race == 2 ~ "Black",
                              race == 3 ~ "Hispanic",
                              race == 4 ~ "Asian",
    ...
    # compare to non-private benchmarks (true weights and unweighted) and store
    for (race in unique(noisy.weighted.res.arb$race)) {
      support.noisy.share <- noisy.weighted.res.arb$share[noisy.weighted.res.arb$race==race & noisy.weighted.res.arb$assault_rifle_ban=="Support"]
      oppose.noisy.share <- noisy.weighted.res.arb$share[noisy.weighted.res.arb$race==race & noisy.weighted.res.arb$assault_rifle_ban=="Oppose"]
      noisy.dif <- support.noisy.share - oppose.noisy.share
      
      support.true.share <- true.weighted.res.arb$share[true.weighted.res.arb$race==race & true.weighted.res.arb$assault_rifle_ban=="Support"]
      oppose.true.share <- true.weighted.res.arb$share[true.weighted.res.arb$race==race & true.weighted.res.arb$assault_rifle_ban=="Oppose"]
      true.dif <- support.true.share - oppose.true.share
      
      support.unweighted.share <- unweighted.res.arb$share[unweighted.res.arb$race==race & unweighted.res.arb$assault_rifle_ban=="Support"]
      oppose.unweighted.share <- unweighted.res.arb$share[unweighted.res.arb$race==race & unweighted.res.arb$assault_rifle_ban=="Oppose"]
      unweighted.dif <- support.unweighted.share - oppose.unweighted.share
      
      results.arb[i,] <- c(race, epsilon, iter, noisy.dif, true.dif, unweighted.dif)
      i <- i + 1
    }
  }
}
...
names(results.arb.df) <- c("race", "epsilon", "iteration", "two_party_dif_noisy_weight", 
...
avg.results.arb.df <- results.arb.df %>% group_by(epsilon, race) %>% 
  summarise(avg_two_party_dif_noisy_weight = mean(two_party_dif_noisy_weight),
            avg_two_party_dif_true_weight = mean(two_party_dif_true_weight),
            avg_two_party_dif_no_weight = mean(two_party_dif_no_weight)) %>% 
...
avg.results.arb.df$race <- factor(avg.results.arb.df$race, 
                                       levels = c("No HS","HS graduate","Some college",
                                                  "2-year degree","4-year degree",
                                                  "Post-graduate degree"))

assault_rifle_ban_difs_race <- ggplot(data=avg.results.arb.df, aes(x=epsilon)) + 
  geom_line(aes(y=(avg_two_party_dif_noisy_weight-avg_two_party_dif_true_weight)*100)) +
